...
- MDIC COMEX table: `gold_comex_flows` — one row per `(flow, reference_year, reference_month, ncm_code, country_code, state_acronym, transport_route_code)` (the transport route `via` is part of the grain; `via_name` via the `comex_via` seed). The currency conventions are applied over `VL_FOB` (US$): `val_yearfx_*` at the registration month's FX, and the BRL-routed `val_real_{ipca,igpm,igpdi}_*` converting US$→BRL at the month's FX, deflating by the BCB chain and reconverting at the current FX (**monthly** deflation, not annual, because the grain is monthly). `val_real_cpi_usd` skips every FX step — the source value already IS dollars — which is why it is the one real column here with no pre-1994 gap.
- UN Comtrade table: `gold_comtrade_flows` — **global** bilateral trade, one row per `(flow, reference_year, reporter_code, partner_code, cmd_code)`. Same conventions over `primaryValue` (US$), but **annual** deflation (year-average FX, year-end inflation index — like PEVS) because the grain is annual. Bilateral geography: `reporter` + `partner` (both M49 → name/ISO3). No double-counting (World dropped in Silver), so `SUM` over partners is the true bilateral total.
...
- Monetary conventions (applicable to any monetary Gold table). They are **not** a `correction × currency` product: an index measures one economy's prices, so it can only correct that economy's money.
  - `val_yearfx_*` — nominal value converted at the year-average FX. NULL for foreign currencies pre-1994.
  - `val_real_{ipca,igpm,igpdi}_{brl,usd,eur}` — deflated by the IPCA / IGP-M / IGP-DI chain and, for a foreign currency, converted at **today's** FX. Measures **Brazilian** purchasing power, merely printed in that currency.
  - `val_real_cpi_usd` / `val_real_hicp_eur` — converted at the **record's** FX and then deflated by US CPI-U / euro-area HICP. This is the dollar (or euro) corrected by its own inflation. Behind the `enable_foreign_inflation` build-order gate; NULL until the first `embrapa ingest foreign-inflation`.
  - The two readings diverge by however much the REAL exchange rate moved, and neither corrects the other. `val_real_cpi_brl` and friends do not exist by design — see `PLANS/correcao_inflacionaria_multimoeda.md`. **Use any `val_real_*` for cross-year comparisons**, but say which one on screen.
...
A push to `main` that touches `dbt/**`, `config.py` or the workflow file itself triggers a prod Silver/Gold build via Workload Identity Federation; a **twice-weekly schedule** (Mondays and Thursdays, 11:30 UTC trigger — GitHub starts it hours later, measured 3h35–9h58) also rebuilds prod so ingested Bronze reaches Silver/Gold/serving without a code push. It was daily until 2026-08-26: the build was 98.9% of the billed BigQuery bytes, mostly rebuilding identical inputs. For data that should not wait for the next slot (a backfill, a one-off ingest), dispatch it by hand: `gh workflow run dbt-build-prod.yml --ref main` — a plain build is enough, since every Silver/Gold model is `materialized='table'` except the year-agnostic incremental `silver_ibge_pevs`. The repo variables `DBT_ENABLE_CURATION` and `DBT_ENABLE_FOREIGN_INFLATION` (both `true` in prod) are handed to dbt in ONE `--vars` mapping — dbt keeps only the last `--vars` flag, and until v1.88.4 the workflow passed one per variable, so the curation flag never arrived (harmless only because `dbt_project.yml` also defaults `enable_curation: true`; see [docs/operations_runbook.md](docs/operations_runbook.md)). Gold snapshots remain manual (`make dbt-build-prod-with-backup` locally, before release boundaries).
