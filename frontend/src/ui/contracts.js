...
// @property {({years:number[],partial:number[],months:Object.<string,number[]>,valuedRange:(number[]|null),coveredBy:string[],share:null}|null)} [valueGap]  `coveredBy` (v1.81.0) = the other corrections, same currency, that DO value every whole year of the gap, asked of the data (PAM/PPM in IPCA: IGP-DI covers 1974–1979 once ingested from 1974); [] when none.  Every banco: the periods of the history the ACTIVE convention cannot value — COMEX month by month (the latest month before its deflator index is ingested); the annual bancos year by year (IBGE/COMTRADE deflate by the year-end index: PAM and PPM have no IPCA in 1974–1979, the dashboard's default convention); € sem correção before 1999 everywhere. `partial` = years with some valued period, `months` names COMEX's missing months (2026 → [8]), `valuedRange` = first and last year WITH value (lets the note say why: an index series that starts later vs one not ingested yet). Global — a missing index nulls every row of its period — so exact for any client-side selection; no `share` (it depends on the selection). The screens MUST name it (window.ValueGapNote, cut to the period by window.windowValueGap).
...
// @property {({years:number[],partial:number[],share:(number|null)}|null)} valueGap  years inside the window the convention cannot value (€ sem correção before 1999; the latest COMEX month before its deflator index is ingested), which of them are missing only IN PART (`partial` — the note says "parte de 2026", never "2026"), and the share of the window's trade — in the declared US$ — they leave out of the sum; null when nothing is missing. The view MUST name them (window.ValueGapNote).
...
// @property {({years:number[],partial:number[],share:(number|null)}|null)} valueGap  years with months the convention cannot value (€ sem correção before 1999; the latest COMEX month before its deflator index is ingested) — those months are null in `matrix` and out of `monthlyAvg`; `partial` = the years with some valued month. The view MUST name them (window.ValueGapNote).
// @property {string}  weightUnit         Volume unit — 'mil t' (net weight).
// @property {number[]} years
// @property {number[]} months           [1..12]
// @property {Object.<number,(number|null)[]>} matrix  year → 12 monthly VALUE (`unit` mi) values. 0 = no trade recorded that month; null = trade the convention cannot value (see valueGap) — never drawn as a zero.
