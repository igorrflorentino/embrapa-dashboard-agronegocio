...
// @property {Object[]} [qualityByUf]       Per-UF not_ok share (tile-map shaped). NOT SERVED today (no endpoint; applyFilters defaults it to []). If revived: `not_ok` means "anything but OK", which since v1.49.0 sweeps in UNSCORED — rows with no basis to evaluate, not defects — so a map of it would paint cube sparsity as damage. Exclude UNSCORED from the numerator.
