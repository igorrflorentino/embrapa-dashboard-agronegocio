...
// @property {({years:number[],partial:number[],months:Object.<string,number[]>,share:null}|null)} [valueGap]  COMEX only: the months of the history the ACTIVE convention cannot value (the latest month before its deflator index is ingested; € sem correção before 1999). `partial` = years with some valued month, `months` names their missing ones (2026 → [8]). Month-level and global — a missing index nulls every row of its month — so exact for any client-side selection; no `share` (it depends on the selection). The screens MUST name it (window.ValueGapNote, cut to the period by window.windowValueGap).
