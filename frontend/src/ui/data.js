...
window.QUALITY_FLAGS = [
  ...
  { id: 'UNSCORED',             label: 'Não avaliada',                            color: 'var(--fg-4)',   desc: 'O detector de preço implícito não teve base para examinar esta linha. São quatro situações: (1) o município mediu produção ZERO no ano — o "-" do IBGE, um zero de verdade, e sem quantidade não há preço a conferir; (2) o registro é pequeno pelas DUAS medidas — o valor, e a quantidade cotada ao preço mediano do produto, ficam abaixo do piso de materialidade (R$ 100 mil / US$ 100 mil), onde o arredondamento domina o preço; (3) o produto tem menos de 100 observações, amostra pequena demais para confiar na mediana; (4) no COMEX, o peso líquido declarado é zero, e o preço é conferido em US$/kg. No rebanho (efetivo, em cabeças) o motivo é estrutural: um estoque não tem valor, logo não tem preço. Não é um defeito do registro — é a ausência de base para avaliá-lo. Distinta de "Normais", que significa examinada e aprovada. Costuma ser a marca da MAIORIA das linhas e de quase nada do valor: no PEVS, 78,4% das linhas e 0,4% do dinheiro; nos outros bancos, menos ainda do dinheiro.' },
  { id: 'MISSING_VALUE',        label: 'Valor financeiro ausente',                color: 'var(--warn)',   desc: 'O valor financeiro do registro (FOB, vendas, faturamento, etc.) veio em branco na fonte; a quantidade existe. Nas pesquisas do IBGE esta marca não aparece: quando o SIDRA não tem o dado ("..."), falta a célula inteira — quantidade e valor juntos — e a linha não entra no acervo. Na PAM, em geral é um município que ainda não existia no ano, ou um produto que a pesquisa não cobria naquele ano; no PEVS, um município que não registrou nenhuma extração vegetal no ano (o IBGE não publica zero para o município inteiro, publica "..."). Por isso esses municípios não pesam nos totais — e o número deles cresce com o tempo, porque a extração, sobretudo de lenha, vem sumindo do Sudeste e do Sul.' },
  { id: 'MISSING_QUANTITY',     label: 'Quantidade ausente',                      color: 'var(--info)',   desc: 'A quantidade do registro (m³, kg, saca, cabeças, etc.) veio em branco na fonte; o valor existe. Na prática não aparece hoje em nenhum banco: nas pesquisas do IBGE a ausência é da célula inteira (ver "Valor financeiro ausente"), e no comércio exterior, onde o preço é conferido por quilo, o que se marca é a falta do peso — "Peso ausente".' },
  { id: 'MISSING_WEIGHT',       label: 'Peso ausente',                            color: 'var(--viz-4)',  desc: 'Registro de comércio exterior com valor e sem peso líquido — impede o cálculo de preço médio por quilo (US$/kg), que é como o detector confere o preço. Aparece no COMTRADE: 3,9% das linhas, quase todas (92%) do capítulo 44 (madeira), cuja quantidade vem em outras unidades.' },
  ...
  { id: 'ISOLATED_SPIKE',       label: 'Pico isolado no tempo',                   color: 'var(--viz-1)',  desc: 'Um valor grande registrado num único ano — sem produção no ano anterior nem no seguinte, no mesmo município e produto — que sozinho (ou junto com outros casos iguais no mesmo estado e ano) faz a série do estado saltar: o total estadual do ano fica pelo menos 1,5 vez acima da média dos anos vizinhos, e os registros isolados explicam ao menos metade desse salto. O preço pode estar perfeitamente plausível, por isso o detector de preço não o marca. Não é prova de erro — uma colheita ou exploração pontual pode ser real —, mas é o padrão de um produto lançado na tabela ou no município errado. Antes de usar aquele ano da série do estado, olhe o registro. Exemplo: Ortigueira e Telêmaco Borba (PR), que registram madeira nativa só em 2011 e fazem sozinhos 46,8% do Paraná naquele ano. Ocorre só nas pesquisas do IBGE (PAM, PEVS, PPM).' },
