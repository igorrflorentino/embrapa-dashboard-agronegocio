...
window.QUALITY_FLAGS = [
  { id: 'OK',                   label: 'Normais',                                 color: 'var(--ok)',     desc: 'O registro tem quantidade e valor, e o sistema conseguiu conferir se os dois combinam. A conferência divide o valor pela quantidade e compara o preço por unidade com o preço típico daquele produto. Nesta linha, nada fugiu do normal.' },
  ...
  { id: 'UNSCORED',             label: 'Não avaliada',                            color: 'var(--fg-4)',   desc: 'O sistema não teve como conferir esta linha, o que não quer dizer que ela tenha algum problema. Os casos mais comuns são o município que não produziu nada naquele ano, o registro pequeno demais para uma comparação confiável (abaixo de R$ 100 mil ou US$ 100 mil), o produto com poucos registros para se saber qual é o preço típico e os rebanhos, que são contados em cabeças e não têm preço. É a situação da maioria das linhas, mas elas somam uma parte muito pequena do valor total.' },
  { id: 'MISSING_VALUE',        label: 'Valor financeiro ausente',                color: 'var(--warn)',   desc: 'A fonte informou a quantidade, mas deixou o valor em branco. Nas pesquisas do IBGE isso não acontece, porque quando falta informação falta o registro inteiro, e ele nem entra na base. Um município que não teve nenhuma extração vegetal no ano, por exemplo, simplesmente não aparece.' },
  { id: 'MISSING_QUANTITY',     label: 'Quantidade ausente',                      color: 'var(--info)',   desc: 'A fonte informou o valor, mas deixou a quantidade em branco. Hoje isso não aparece em nenhuma base, porque no IBGE falta sempre o registro inteiro e, no comércio exterior, o que falta é o peso, que tem marca própria.' },
  { id: 'MISSING_WEIGHT',       label: 'Peso ausente',                            color: 'var(--viz-4)',  desc: 'Um registro de comércio exterior que informa o valor, mas não o peso. Sem o peso não dá para calcular o preço por quilo, que é como esses registros são conferidos. Acontece no COMTRADE, quase sempre com madeira, que costuma ser medida em outras unidades.' },
  { id: 'INCOMPLETE',           label: 'Incompleto',                              color: 'var(--viz-7)',  desc: 'O registro não tem nem quantidade nem valor, então não há o que analisar.' },
  { id: 'OUTLIER_QUANTITY',     label: 'Quantidade atípica (válida)',             color: 'var(--viz-3)',  desc: 'A quantidade é muito maior do que esse produto costuma registrar, mas o preço por unidade continua dentro do esperado. Por isso o registro é tratado como um número grande de verdade, como o de um grande produtor, e não como erro. A comparação considera toda a história do produto, então quem cresceu muito tende a ter mais registros assim nos anos recentes.' },
  { id: 'PROBLEMATIC_QUANTITY', label: 'Quantidade problemática (provável erro)', color: 'var(--viz-9)',  desc: 'O preço por unidade ficou mais de 100 vezes acima ou abaixo do preço típico do produto, e a quantidade é o número que parece fora do lugar. Quase sempre é erro de digitação na fonte. O caso mais comum é o peso registrado como 1 kg numa carga de alto valor, e também aparecem pesos com zeros a mais.' },
  { id: 'OUTLIER_VALUE',        label: 'Valor atípico (válido)',                  color: 'var(--viz-5)',  desc: 'O valor é muito maior do que esse produto costuma registrar, mas o preço por unidade continua dentro do esperado. Por isso o registro é tratado como um número grande de verdade, como o de um grande produtor, e não como erro. A comparação considera toda a história do produto, então quem cresceu muito tende a ter mais registros assim nos anos recentes.' },
  { id: 'PROBLEMATIC_VALUE',    label: 'Valor problemático (provável erro)',      color: 'var(--err)',    desc: 'O preço por unidade ficou mais de 100 vezes acima ou abaixo do preço típico do produto, e o valor é o número que parece fora do lugar. Quase sempre é erro de digitação na fonte, como um valor com zeros a mais ou a menos.' },
  ...
  { id: 'ISOLATED_SPIKE',       label: 'Pico isolado no tempo',                   color: 'var(--viz-1)',  desc: 'Um valor grande que aparece num único ano, sem produção no ano anterior nem no seguinte, e que faz o total do estado dar um salto justamente naquele ano. O preço pode parecer normal, mas esse padrão costuma indicar um registro lançado no lugar errado pela fonte. Um exemplo é a madeira nativa registrada em Ortigueira e Telêmaco Borba (PR) só em 2011, que sozinha equivale a quase metade do Paraná naquele ano. Vale olhar o registro antes de usar esse ano na série do estado.' },
  // PAM-only: a SIDRA source error preserved faithfully and now surfaced in-product.
  { id: 'AREA_INCONSISTENT',    label: 'Área inconsistente (plantada < colhida)', color: 'var(--viz-6)',  desc: 'A área plantada informada pelo IBGE é menor que a área colhida, o que não é possível, já que não se colhe mais do que se planta. É um erro da própria fonte. O registro fica como veio e recebe esta marca, em vez de ser corrigido sem aviso. Acontece só nas lavouras (PAM).' },
  // Reserved for a FUTURE auto-fill pipeline (accepted-but-absent — render 0 today).
  { id: 'INFERRED_QUANTITY',    label: 'Quantidade inferida',                     color: 'var(--viz-8)',  reserved: true, desc: 'Reservada para uma etapa futura que completaria automaticamente uma quantidade que veio em branco. Ainda não é usada.' },
  { id: 'INFERRED_VALUE',       label: 'Valor financeiro inferido',               color: 'var(--viz-10)', reserved: true, desc: 'Reservada para uma etapa futura que completaria automaticamente um valor que veio em branco. Ainda não é usada.' },
];
