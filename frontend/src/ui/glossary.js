...
const GLOSSARY = {
  ibge_pevs: {
    ...
    terms: [
      ...
      { term: 'gold_pevs_production', cat: 'Tabela', tag: 'Base analítica', short: 'Tabela da base analítica. Cada linha é uma combinação de ano, UF, município e produto. É a origem de todos os números das telas.' },
      { term: 'val_yearfx_*',          cat: 'Coluna', tag: 'Base analítica',     short: 'Valor da época, sem correção pela inflação. Em reais é o próprio valor da fonte, e em dólar ou euro é convertido pelo câmbio médio do ano. Serve para conferir um ano, e não para comparar anos diferentes.' },
      { term: 'val_real_ipca_*',       cat: 'Coluna', tag: 'Base analítica',     short: 'Valor trazido a reais de hoje pela inflação medida pelo IPCA, o primeiro índice oferecido para valores em reais. O painel abre sem correção, e corrigir é escolha de quem pesquisa. A tabela também guarda versões em dólar e em euro, que convertem pelo câmbio de hoje um valor corrigido pela inflação brasileira, mas o painel não as oferece.' },
      { term: 'val_real_igpm_*',       cat: 'Coluna', tag: 'Base analítica',     short: 'O mesmo valor, corrigido pelo IGP-M da FGV, índice comum em séries de preços no atacado. O IGP-M só começa em 1989, então a coluna fica vazia nos anos anteriores, o que não é defeito. Nesses anos o IPCA cobre a partir de 1980, e o IGP-DI cobre todo o acervo, desde 1974.' },
      { term: 'val_real_cpi_usd',      cat: 'Coluna', tag: 'Base analítica',     short: 'Valor em dólares de hoje pelo poder de compra americano. O valor é convertido pelo câmbio do ano de registro e depois corrigido pelo CPI, o índice de preços ao consumidor dos EUA. É diferente de val_real_ipca_usd, que corrige em reais pelo IPCA e converte pelo câmbio de hoje, e por isso mede o poder de compra brasileiro escrito em dólares. Nas bases de comércio exterior, cujo valor já vem em dólares, esta coluna não passa por câmbio nenhum.' },
      { term: 'val_real_hicp_eur',     cat: 'Coluna', tag: 'Base analítica',     short: 'O equivalente em euros. O valor é convertido pelo câmbio do ano e corrigido pelo HICP, o índice de preços ao consumidor da zona do euro, publicado pelo BCE e pelo Eurostat. Fica vazio antes de 1999, porque o euro ainda não existia.' },
      { term: 'data_quality_flag',     cat: 'Coluna', tag: 'qualidade', short: 'Marca que diz se o sistema conseguiu conferir a linha e o que encontrou. A conferência compara o preço por unidade, que é o valor dividido pela quantidade, com o preço típico do produto. A maioria das linhas fica sem conferência por falta de base, o que não é defeito. Cada marca é explicada na perspectiva Qualidade dos dados. Nesta base também aparece a marca Pico num único ano, um valor grande que aparece num só ano e faz a série do estado dar um salto.' },
    ],
  },
  ...
  ibge_pam: {
    ...
    terms: [
      ...
      { term: 'gold_pam_production', cat: 'Tabela', tag: 'Base analítica', short: 'Tabela da base analítica. Cada linha é uma combinação de ano, UF, município e lavoura, com área, produção, rendimento e valor.' },
      { term: 'data_quality_flag',     cat: 'Coluna', tag: 'qualidade', short: 'Marca que diz se o sistema conseguiu conferir a linha e o que encontrou. A conferência compara o preço por unidade, que é o valor dividido pela quantidade, com o preço típico do produto. A maioria das linhas fica sem conferência por falta de base, o que não é defeito. Cada marca é explicada na perspectiva Qualidade dos dados. Nesta base também aparecem as marcas Pico num único ano, um valor grande que aparece num só ano e faz a série do estado dar um salto, e Área plantada menor que a colhida.' },
    ],
  },
  ...
  ibge_ppm: {
    ...
    terms: [
      ...
      { term: 'gold_ppm_production', cat: 'Tabela', tag: 'Base analítica', short: 'Tabela da base analítica. Cada linha é uma combinação de ano, UF, município e rebanho/produto, com quantidade (na unidade do produto) e — para a produção animal — valor.' },
      { term: 'data_quality_flag',     cat: 'Coluna', tag: 'qualidade', short: 'Marca que diz se o sistema conseguiu conferir a linha e o que encontrou. A conferência compara o preço por unidade, que é o valor dividido pela quantidade, com o preço típico do produto. A maioria das linhas fica sem conferência por falta de base, o que não é defeito. Cada marca é explicada na perspectiva Qualidade dos dados. Os rebanhos ficam sempre sem conferência, porque são contados em cabeças e não têm preço. Nesta base também aparece a marca Pico num único ano, um valor grande que aparece num só ano e faz a série do estado dar um salto.' },
    ],
  },
  ...
  mdic_comex: {
    ...
    terms: [
      ...
      { term: 'Via',          cat: 'Coluna', short: 'Modalidade de transporte (marítima, aérea, rodoviária, ferroviária, fluvial, dutos). Existe na base bruta, mas é somada nas tabelas pré-agregadas que o painel lê — não é um filtro disponível no painel.' },
      ...
      { term: 'val_yearfx_* · val_real_*', cat: 'Coluna', tag: 'Base analítica', short: 'O valor de origem do comércio é o FOB em dólares. val_yearfx_usd é o próprio valor da fonte, sem conversão, e as versões em reais e em euros usam o câmbio médio do ano. As colunas val_real_* trazem os valores a preços de hoje, para comparar anos diferentes. Em dólares o índice oferecido é o CPI dos EUA (val_real_cpi_usd), que não passa por câmbio nenhum, porque o valor já vem em dólares.' },
      { term: 'gold_comex_flows', cat: 'Tabela', tag: 'Base analítica', short: 'Tabela da base analítica. Cada linha é uma combinação de ano-mês, UF, NCM, país, via e fluxo.' },
      { term: 'data_quality_flag', cat: 'Coluna', tag: 'qualidade', short: 'Marca que diz se o sistema conseguiu conferir a linha e o que encontrou. A conferência compara o preço por unidade, que é o valor dividido pela quantidade, com o preço típico do produto. A maioria das linhas fica sem conferência por falta de base, o que não é defeito. Cada marca é explicada na perspectiva Qualidade dos dados. Aqui a quantidade usada é o peso líquido, e o preço conferido é o valor em dólares por quilo.' },
    ],
  },
  ...
  un_comtrade: {
    ...
    terms: [
      ...
      { term: 'gold_comtrade_flows', cat: 'Tabela', tag: 'Base analítica', short: 'Tabela da base analítica. Cada linha é uma combinação de ano, país reporter, país parceiro, regime aduaneiro, HS6 e fluxo. (O Brasil declara hoje apenas no regime de totais C00, mas a coluna faz parte do grão.)' },
      { term: 'data_quality_flag', cat: 'Coluna', tag: 'qualidade', short: 'Marca que diz se o sistema conseguiu conferir a linha e o que encontrou. A conferência compara o preço por unidade, que é o valor dividido pela quantidade, com o preço típico do produto. A maioria das linhas fica sem conferência por falta de base, o que não é defeito. Cada marca é explicada na perspectiva Qualidade dos dados. Aqui a quantidade usada é o peso líquido, e o preço conferido é o valor em dólares por quilo. Quando o país não informa o peso, o que acontece sobretudo com madeira, a linha recebe a marca Sem peso e o valor continua valendo.' },
    ],
  },
  ...
  curadoria: {
    ...
    sub:   'Atributos derivados: conhecimento do pesquisador (industrialização) + tabela de referência (tipo de mercado)',
    kind:  'tema',
    terms: [
      { term: 'Engenharia de atributos', cat: 'Conceito', short: 'Camada de colunas derivadas construídas sobre os dados: o nível de industrialização (editável pelo pesquisador) e o tipo de mercado (definido por uma tabela de referência). Alimenta as análises curadas no modo multi-fonte.' },
      ...
      { term: 'Consumo · Processamento', cat: 'Dimensão', short: 'Os dois destinos do bem: consumo final, ou transformação/beneficiamento industrial. É a finalidade que a tabela de referência atribuiria a cada par regime × fluxo. RECURSO SUSPENSO (ver "Finalidade econômica").' },
      ...
      { term: 'Catálogo (Cadastro de produtos)', cat: 'Operação', short: 'A lista, curada pelo pesquisador, de quais produtos entram e saem do painel, com seu agrupamento e ciclo de vida (exibir ou ocultar). Editada na tela "Cadastro de produtos"; toda alteração fica registrada com o e-mail do autor.' },
  ...
  metodos: {
    ...
    terms: [
      ...
      { term: 'Valor nominal × valor real', cat: 'Método', short: 'O valor nominal é o valor em dinheiro da época. O valor real é o mesmo valor trazido a preços de hoje pela inflação, para comparar anos diferentes de forma justa. Em moeda estrangeira existem duas maneiras de fazer essa correção, e elas dão números diferentes (veja "Corrigir em qual economia?").' },
      { term: 'Deflacionar', cat: 'Método', short: 'Tirar o efeito da inflação de uma série de valores, trazendo anos diferentes para o poder de compra de um mesmo ano. Sem essa correção, um valor de 1990 parece muito menor do que realmente era, porque os preços subiram muito desde então. É o que fazem as colunas val_real_*.' },
      { term: 'Corrigir em qual economia?', cat: 'Método', short: 'Um índice de inflação mede os preços de uma única economia, então só corrige a moeda dessa economia. Por isso o índice acompanha a moeda. Em reais você escolhe entre IPCA, IGP-M e IGP-DI, em dólares o CPI e em euros o HICP. Um valor em dólares corrigido pelo CPI é convertido pelo câmbio do ano de registro e trazido a dólares de hoje pela inflação americana. Outra leitura, que o painel não oferece, corrige em reais pelo IPCA e converte pelo câmbio de hoje (val_real_ipca_usd). Ela mede o poder de compra brasileiro escrito em dólares, dá outro número e continua nas tabelas finais para quem precisar.' },
      { term: 'IPCA · IGP-M · IGP-DI', cat: 'Método', short: 'Os três índices brasileiros usados para trazer valores antigos a preços de hoje, e por isso só corrigem valores em reais. O IPCA, do IBGE, mede a inflação ao consumidor e é o mais usado. O IGP-M e o IGP-DI, da FGV, dão mais peso aos preços no atacado. Cada um alcança um período diferente. O IPCA começa em 1980, o IGP-M em 1989, e o IGP-DI cobre todo o acervo, desde 1974.' },
      { term: 'CPI (EUA)', cat: 'Método', short: 'Índice de preços ao consumidor dos Estados Unidos (Consumer Price Index for All Urban Consumers, série CUUR0000SA0), publicado todo mês pelo BLS desde 1913. É o índice que corrige valores em dólares. Usá-lo não é o mesmo que corrigir em reais pelo IPCA e converter depois.' },
      { term: 'HICP (zona do euro)', cat: 'Método', short: 'Índice harmonizado de preços ao consumidor da zona do euro, publicado todo mês pelo BCE e pelo Eurostat desde 1996. É o índice que corrige valores em euros. Como o euro só existe desde 1999, não há valor corrigido em euros antes disso.' },
      { term: 'Moeda (R$ · US$ · €)', cat: 'Método', short: 'Você escolhe em qual moeda ver os valores. A produção do IBGE vem em reais e o comércio exterior vem em dólares, e a conversão usa o câmbio do Banco Central. Cada moeda tem os seus índices de inflação, e a faixa mostra só os da moeda escolhida, IPCA, IGP-M ou IGP-DI para reais, CPI para dólares e HICP para euros. Ao trocar de moeda com uma correção ligada, o painel passa para o índice da nova moeda. O painel abre sem correção.' },
