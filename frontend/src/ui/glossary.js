...
const GLOSSARY = {
  ibge_pevs: {
    ...
    terms: [
      ...
      { term: 'data_quality_flag',     cat: 'Coluna', tag: 'qualidade', short: 'Marca de confiabilidade de cada linha. Ela responde DUAS perguntas: a linha está completa (tem quantidade e valor?) e o número é plausível (o preço implícito — valor÷quantidade — está perto da mediana do produto?). Daí: Normais (completa E examinada pelo detector, sem ressalva); valor financeiro ausente, quantidade ausente, incompleta (sem quantidade nem valor); atípica (válida) — bem acima do esperado, mas com preço coerente, um gigante de verdade; problemática (provável erro de digitação) — preço muito fora da mediana; e Não avaliada, a linha que o detector NÃO teve base para examinar (zero medido, valor abaixo do piso de materialidade, ano sem deflator, amostra pequena). ‘Não avaliada’ não é defeito e costuma ser a maioria das linhas — e quase nada do valor. Reserva ainda dois níveis para preenchimento automático futuro (quantidade / valor inferido), hoje sempre vazios; a legenda completa fica na perspectiva Qualidade dos dados.' },
    ],
  },
  ...
  ibge_pam: {
    ...
    terms: [
      ...
      { term: 'data_quality_flag',     cat: 'Coluna', tag: 'qualidade', short: 'Marca de confiabilidade de cada linha. Ela responde DUAS perguntas: a linha está completa (tem quantidade e valor?) e o número é plausível (o preço implícito — valor÷quantidade — está perto da mediana do produto?). Daí: Normais (completa E examinada pelo detector, sem ressalva); valor financeiro ausente, quantidade ausente, incompleta (sem quantidade nem valor); atípica (válida) — bem acima do esperado, mas com preço coerente, um gigante de verdade; problemática (provável erro de digitação) — preço muito fora da mediana; área inconsistente — área plantada menor que a colhida, agronomicamente impossível, um erro do SIDRA preservado fielmente em vez de corrigido em silêncio; e Não avaliada, a linha que o detector NÃO teve base para examinar (zero medido, valor abaixo do piso de materialidade, ano sem deflator, amostra pequena). ‘Não avaliada’ não é defeito e costuma ser a maioria das linhas — e quase nada do valor. Reserva ainda dois níveis para preenchimento automático futuro (quantidade / valor inferido), hoje sempre vazios; a legenda completa fica na perspectiva Qualidade dos dados.' },
    ],
  },
  ...
  ibge_ppm: {
    ...
    terms: [
      ...
      { term: 'data_quality_flag',     cat: 'Coluna', tag: 'qualidade', short: 'Marca de confiabilidade de cada linha. Ela responde DUAS perguntas: a linha está completa (tem quantidade e valor?) e o número é plausível (o preço implícito — valor÷quantidade — está perto da mediana do produto?). Daí: Normais (completa E examinada pelo detector, sem ressalva); valor financeiro ausente, quantidade ausente, incompleta; atípica (válida) — bem acima do esperado, mas com preço coerente, um gigante de verdade; problemática (provável erro de digitação) — preço muito fora da mediana; e Não avaliada, a linha que o detector NÃO teve base para examinar (zero medido, valor abaixo do piso de materialidade, ano sem deflator, amostra pequena, e no rebanho — um efetivo em cabeças é um estoque: não tem valor, logo não tem preço a avaliar). ‘Não avaliada’ não é defeito e costuma ser a maioria das linhas — e quase nada do valor. Reserva ainda dois níveis para preenchimento automático futuro (quantidade / valor inferido), hoje sempre vazios; a legenda completa fica na perspectiva Qualidade dos dados.' },
    ],
  },
  ...
  mdic_comex: {
    ...
    terms: [
      ...
      { term: 'data_quality_flag', cat: 'Coluna', tag: 'qualidade', short: 'Marca de confiabilidade de cada linha. Ela responde DUAS perguntas: a linha está completa (tem quantidade e valor?) e o número é plausível (o preço implícito — valor÷quantidade — está perto da mediana do produto?). Daí: Normais (completa E examinada pelo detector, sem ressalva); valor financeiro ausente, peso ausente (sem peso líquido), incompleta; atípica (válida) — bem acima do esperado, mas com preço coerente, um gigante de verdade; problemática (provável erro de digitação) — preço muito fora da mediana (ex.: peso = 1 kg numa carga cara); e Não avaliada, a linha que o detector NÃO teve base para examinar (zero medido, valor abaixo do piso de materialidade, ano sem deflator, amostra pequena). ‘Não avaliada’ não é defeito e costuma ser a maioria das linhas — e quase nada do valor. Reserva ainda dois níveis para preenchimento automático futuro (peso / valor inferido), hoje sempre vazios; a legenda completa fica na perspectiva Qualidade dos dados.' },
    ],
  },
  ...
  un_comtrade: {
    ...
    terms: [
      ...
      { term: 'data_quality_flag', cat: 'Coluna', tag: 'qualidade', short: 'Marca de confiabilidade de cada linha. Ela responde DUAS perguntas: a linha está completa (tem quantidade e valor?) e o número é plausível (o preço implícito — valor÷quantidade — está perto da mediana do produto?). Daí: Normais (completa E examinada pelo detector, sem ressalva); valor financeiro ausente, quantidade ausente, incompleta; atípica (válida) — bem acima do esperado, mas com preço coerente, um gigante de verdade; problemática (provável erro de digitação) — preço muito fora da mediana (ex.: peso = 1 kg em chapas de madeira de alto valor); e Não avaliada, a linha que o detector NÃO teve base para examinar (zero medido, valor abaixo do piso de materialidade, ano sem deflator, amostra pequena). ‘Não avaliada’ não é defeito e costuma ser a maioria das linhas — e quase nada do valor. Reserva ainda dois níveis para preenchimento automático futuro (quantidade / valor inferido), hoje sempre vazios; a legenda completa fica na perspectiva Qualidade dos dados.' },
    ],
  },
