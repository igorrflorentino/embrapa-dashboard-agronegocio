...
const GLOSSARY = {
  ibge_pevs: {
    ...
    terms: [
      ...
      { term: 'val_real_ipca_*',       cat: 'Coluna', tag: 'gold',     short: 'Valor projetado para hoje pela cadeia IPCA — o primeiro índice oferecido para valores em R$. O painel abre sem correção (valores nominais); a correção é escolha do pesquisador. As variantes em US$ e € (inflação do Brasil ao câmbio de hoje) existem na tabela, mas o painel não as oferece.' },
      { term: 'val_real_igpm_*',       cat: 'Coluna', tag: 'gold',     short: 'Idem usando o IGP-M (FGV). Alternativa ao IPCA, comum em séries de preços no atacado e de commodities. O IGP-M começa em 1989 — por isso val_real_igpm_* fica NULL para anos anteriores (não é um defeito do pipeline), enquanto o IPCA (desde 1980) e o IGP-DI (desde 1974 neste acervo) preenchem.' },
      { term: 'val_real_cpi_usd',      cat: 'Coluna', tag: 'gold',     short: 'Dólares de hoje pelo PODER DE COMPRA AMERICANO: o valor é convertido pelo câmbio do ano de registro e corrigido pelo CPI (índice de preços ao consumidor dos EUA, do BLS). É uma coluna DIFERENTE de val_real_ipca_usd, que deflaciona em reais pelo IPCA e converte pelo câmbio de hoje — aquela mede poder de compra brasileiro e só está impressa em dólares. Num banco de aduana (COMEX/Comtrade), cujo valor de origem já é US$, esta coluna não passa por câmbio nenhum.' },
      { term: 'val_real_hicp_eur',     cat: 'Coluna', tag: 'gold',     short: 'O equivalente em euros: convertido pelo câmbio do ano e corrigido pelo HICP (índice harmonizado de preços ao consumidor da zona do euro, publicado pelo BCE/Eurostat). NULL antes de 1999 — o euro não existia.' },
      { term: 'data_quality_flag',     cat: 'Coluna', tag: 'qualidade', short: 'Marca de confiabilidade de cada linha. Ela responde DUAS perguntas: a linha está completa (tem quantidade e valor?) e o número é plausível (o preço implícito — valor÷quantidade — está perto da mediana do produto?). Daí: Normais (completa E examinada pelo detector, sem ressalva); valor financeiro ausente, quantidade ausente, incompleta (sem quantidade nem valor); atípica (válida) — muito acima do que o produto costuma registrar ao longo de toda a sua história, com preço dentro de 100× da mediana, em geral um gigante de verdade; problemática (provável erro de digitação) — preço mais de 100× acima ou abaixo da mediana; pico isolado no tempo — valor grande num ano só, sem produção antes nem depois, que faz a série do estado saltar (o preço pode estar certo; é o padrão de um lançamento na tabela ou no município errado); e Não avaliada, a linha que o detector NÃO teve base para examinar (zero medido, registro pequeno pelo valor e pela quantidade — abaixo do piso de materialidade —, amostra pequena). ‘Não avaliada’ não é defeito e costuma ser a maioria das linhas — e quase nada do valor. Reserva ainda dois níveis para preenchimento automático futuro (quantidade / valor inferido), hoje sempre vazios; a legenda completa fica na perspectiva Qualidade dos dados.' },
    ],
  },
  ...
  ibge_pam: {
    ...
    terms: [
      ...
      { term: 'data_quality_flag',     cat: 'Coluna', tag: 'qualidade', short: 'Marca de confiabilidade de cada linha. Ela responde DUAS perguntas: a linha está completa (tem quantidade e valor?) e o número é plausível (o preço implícito — valor÷quantidade — está perto da mediana do produto?). Daí: Normais (completa E examinada pelo detector, sem ressalva); valor financeiro ausente, quantidade ausente, incompleta (sem quantidade nem valor); atípica (válida) — muito acima do que o produto costuma registrar ao longo de toda a sua história, com preço dentro de 100× da mediana, em geral um gigante de verdade; problemática (provável erro de digitação) — preço mais de 100× acima ou abaixo da mediana; pico isolado no tempo — valor grande num ano só, sem produção antes nem depois, que faz a série do estado saltar (o preço pode estar certo; é o padrão de um lançamento na tabela ou no município errado); área inconsistente — área plantada menor que a colhida, agronomicamente impossível, um erro do SIDRA preservado fielmente em vez de corrigido em silêncio; e Não avaliada, a linha que o detector NÃO teve base para examinar (zero medido, registro pequeno pelo valor e pela quantidade — abaixo do piso de materialidade —, amostra pequena). ‘Não avaliada’ não é defeito e costuma ser a maioria das linhas — e quase nada do valor. Reserva ainda dois níveis para preenchimento automático futuro (quantidade / valor inferido), hoje sempre vazios; a legenda completa fica na perspectiva Qualidade dos dados.' },
    ],
  },
  ...
  ibge_ppm: {
    ...
    terms: [
      ...
      { term: 'data_quality_flag',     cat: 'Coluna', tag: 'qualidade', short: 'Marca de confiabilidade de cada linha. Ela responde DUAS perguntas: a linha está completa (tem quantidade e valor?) e o número é plausível (o preço implícito — valor÷quantidade — está perto da mediana do produto?). Daí: Normais (completa E examinada pelo detector, sem ressalva); valor financeiro ausente, quantidade ausente, incompleta; atípica (válida) — muito acima do que o produto costuma registrar ao longo de toda a sua história, com preço dentro de 100× da mediana, em geral um gigante de verdade; problemática (provável erro de digitação) — preço mais de 100× acima ou abaixo da mediana; pico isolado no tempo — valor grande num ano só, sem produção antes nem depois, que faz a série do estado saltar (o preço pode estar certo; é o padrão de um lançamento na tabela ou no município errado); e Não avaliada, a linha que o detector NÃO teve base para examinar (zero medido, registro pequeno pelo valor e pela quantidade — abaixo do piso de materialidade —, amostra pequena, e no rebanho — um efetivo em cabeças é um estoque: não tem valor, logo não tem preço a avaliar). ‘Não avaliada’ não é defeito e costuma ser a maioria das linhas — e quase nada do valor. Reserva ainda dois níveis para preenchimento automático futuro (quantidade / valor inferido), hoje sempre vazios; a legenda completa fica na perspectiva Qualidade dos dados.' },
    ],
  },
  ...
  mdic_comex: {
    ...
    terms: [
      ...
      { term: 'data_quality_flag', cat: 'Coluna', tag: 'qualidade', short: 'Marca de confiabilidade de cada linha. Ela responde DUAS perguntas: a linha está completa (tem quantidade e valor?) e o número é plausível (o preço implícito — valor÷quantidade — está perto da mediana do produto?). Daí: Normais (completa E examinada pelo detector, sem ressalva); valor financeiro ausente, peso ausente (sem peso líquido), incompleta; atípica (válida) — muito acima do que o produto costuma registrar ao longo de toda a sua história, com preço dentro de 100× da mediana, em geral um gigante de verdade; problemática (provável erro de digitação) — preço mais de 100× acima ou abaixo da mediana (ex.: peso = 1 kg numa carga cara); e Não avaliada, a linha que o detector NÃO teve base para examinar (zero medido, registro pequeno pelo valor e pela quantidade — abaixo do piso de materialidade —, amostra pequena, peso líquido zero). ‘Não avaliada’ não é defeito e costuma ser a maioria das linhas — e quase nada do valor. Reserva ainda dois níveis para preenchimento automático futuro (peso / valor inferido), hoje sempre vazios; a legenda completa fica na perspectiva Qualidade dos dados.' },
    ],
  },
  ...
  un_comtrade: {
    ...
    terms: [
      ...
      { term: 'data_quality_flag', cat: 'Coluna', tag: 'qualidade', short: 'Marca de confiabilidade de cada linha. Ela responde DUAS perguntas: a linha está completa (tem quantidade e valor?) e o número é plausível (o preço implícito — valor÷quantidade — está perto da mediana do produto?). Daí: Normais (completa E examinada pelo detector, sem ressalva); valor financeiro ausente, peso ausente (sem peso líquido — o preço é conferido em US$/kg; quase sempre madeira do capítulo 44), incompleta; atípica (válida) — muito acima do que o produto costuma registrar ao longo de toda a sua história, com preço dentro de 100× da mediana, em geral um gigante de verdade; problemática (provável erro de digitação) — preço mais de 100× acima ou abaixo da mediana (ex.: peso = 1 kg em chapas de madeira de alto valor); e Não avaliada, a linha que o detector NÃO teve base para examinar (zero medido, registro pequeno pelo valor e pela quantidade — abaixo do piso de materialidade —, amostra pequena). ‘Não avaliada’ não é defeito e costuma ser a maioria das linhas — e quase nada do valor. Reserva ainda dois níveis para preenchimento automático futuro (quantidade / valor inferido), hoje sempre vazios; a legenda completa fica na perspectiva Qualidade dos dados.' },
    ],
  },
  ...
  metodos: {
    ...
    terms: [
      ...
      { term: 'Valor nominal × valor real', cat: 'Método', short: 'Nominal = o valor em moeda da época. Real = o mesmo valor trazido a preços de hoje pela inflação, para comparar anos diferentes de forma justa. Em moeda estrangeira há DUAS maneiras de fazer isso e elas dão números diferentes — veja "Corrigir em qual economia?".' },
      { term: 'Deflacionar', cat: 'Método', short: 'Retirar o efeito da inflação de uma série de valores, trazendo valores de anos diferentes para o poder de compra de um mesmo ano de referência. Sem deflacionar, um valor de 1990 parece muito maior que o de hoje só por causa da inflação. É o que fazem as colunas val_real_*.' },
      { term: 'Corrigir em qual economia?', cat: 'Método', short: 'Um índice de inflação mede os preços de UMA economia, então só corrige a moeda DESSA economia. Por isso, no painel, o índice acompanha a moeda: em R$ você escolhe entre IPCA, IGP-M e IGP-DI; em US$, o CPI; em €, o HICP — e em qualquer moeda pode ficar sem correção. Um valor em dólares corrigido pelo CPI é convertido pelo câmbio do ANO de registro e trazido a dólares de hoje pela inflação americana. Existe uma outra leitura, que o painel NÃO oferece: corrigir em reais pelo IPCA e converter pelo câmbio de HOJE (val_real_ipca_usd). Ela mede poder de compra brasileiro apresentado em dólares, e diverge da primeira tanto quanto o câmbio real variou; está nas tabelas Gold para quem precisar dela.' },
      { term: 'IPCA · IGP-M · IGP-DI', cat: 'Método', short: 'Os três índices BRASILEIROS usados para trazer valores antigos a preços de hoje. Corrigem valores em R$. O IPCA (IBGE) mede a inflação ao consumidor e é o mais usado; o IGP-M e o IGP-DI (FGV) pesam mais o atacado — o IGP-M começa em 1989, enquanto IGP-DI e IPCA vêm de antes.' },
      { term: 'CPI (EUA)', cat: 'Método', short: 'Consumer Price Index for All Urban Consumers (CUUR0000SA0), publicado pelo BLS — o índice oficial de inflação ao consumidor dos Estados Unidos, mensal desde 1913. É o índice que corrige valores em DÓLARES: usá-lo é diferente de corrigir em reais pelo IPCA e converter depois.' },
      { term: 'HICP (zona do euro)', cat: 'Método', short: 'Índice Harmonizado de Preços ao Consumidor da zona do euro, publicado pelo BCE/Eurostat (mensal desde 1996). É o índice que corrige valores em EUROS. Como o euro só existe desde 1999, não há valor corrigido em € antes disso.' },
      { term: 'Moeda (R$ · US$ · €)', cat: 'Método', short: 'Você escolhe em qual moeda ver os valores: a produção (IBGE) é nativa em reais, o comércio (COMEX/Comtrade) em dólares, e a conversão usa o câmbio real do BCB. Cada moeda tem os seus índices, porque cada índice corrige apenas a moeda da economia que mede: R$ → IPCA, IGP-M, IGP-DI; US$ → CPI; € → HICP. A faixa só mostra os da moeda escolhida. Ao trocar de moeda com uma correção ligada, o painel passa ao índice da nova moeda; um link antigo com uma combinação que não é mais oferecida é ajustado ao abrir. O painel abre sem correção (valores nominais).' },
