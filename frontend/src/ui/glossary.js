...
const GLOSSARY = {
  ...
  metodos: {
    ...
    terms: [
      ...
      { term: 'Corrigir em qual economia?', cat: 'Método', short: 'Um índice de inflação mede os preços de UMA economia, então só sabe dizer quanto algo valia NAQUELA moeda. Com isso, um valor exibido em dólares pode ser corrigido de duas formas, que respondem perguntas diferentes: (1) corrigir em reais pelo IPCA/IGP-M/IGP-DI e converter pelo câmbio de HOJE — mede poder de compra brasileiro e apenas apresenta o resultado em dólares; (2) converter pelo câmbio do ANO de registro e corrigir pelo CPI — dólares da época trazidos a dólares de hoje. As duas divergem tanto quanto o câmbio real variou no período, e nenhuma corrige a outra. A faixa de convenções separa as duas em bandas e diz, em uma linha, qual está ativa.' },
      { term: 'IPCA · IGP-M · IGP-DI', cat: 'Método', short: 'Os três índices BRASILEIROS usados para trazer valores antigos a preços de hoje. O IPCA (IBGE) mede a inflação ao consumidor e é o padrão do dashboard; o IGP-M e o IGP-DI (FGV) pesam mais o atacado — o IGP-M começa em 1989, enquanto IGP-DI e IPCA vêm de antes.' },
      ...
      { term: 'Moeda (R$ · US$ · €)', cat: 'Método', short: 'Você escolhe em qual moeda ver os valores: a produção (IBGE) é nativa em reais, o comércio (COMEX/Comtrade) em dólares, e a conversão usa o câmbio real do BCB. Nem toda combinação de moeda e correção existe: o IGP-M e o IGP-DI só têm coluna em R$ e €, e cada índice estrangeiro corrige apenas a sua própria moeda (o CPI corrige US$, o HICP corrige €) — não existe "real corrigido pelo CPI". A faixa desabilita as combinações impossíveis e um link antigo que traga uma delas é ajustado ao abrir.' },
