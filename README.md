...
| `UNSCORED` | Não avaliada | the detector had **no basis** to examine the row: measured zero, small by BOTH measures (value and quantity × median price under the materiality floor), fewer than `quality_min_obs` observations for that product, or a COMEX weight of zero. **Not a defect** — a row with nothing to evaluate, not a damaged one |
| `MISSING_VALUE` | Valor financeiro ausente | quantity reported but monetary value missing |
| `MISSING_QUANTITY` | Quantidade ausente | monetary value reported but quantity missing (0 rows today: IBGE absences blank the whole cell, and trade marks the missing weight) |
| `MISSING_WEIGHT` | Peso ausente | trade only — value reported but net weight missing. COMTRADE 79.536 rows (3,9%), 92% of them chapter-44 wood reported in other units; COMEX always reports a weight |
...
| `PROBLEMATIC_VALUE` / `PROBLEMATIC_QUANTITY` | Valor / Quantidade problemática (provável erro) | implied price (value ÷ quantity) >100× or <1/100× the product median ⇒ likely typo, blamed on the more anomalous measure. Two-sided: 57% of `PROBLEMATIC_QUANTITY` rows are quantities BELOW the median (the weight = 1 placeholder), the rest above (a weight with extra digits) |
