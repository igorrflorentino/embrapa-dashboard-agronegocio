...
| `MISSING_WEIGHT` | Peso ausente | COMEX only — value reported but net weight missing (0 rows today: COMEX always reports a weight). COMTRADE's weightless rows land in `MISSING_QUANTITY` or `UNSCORED` instead |
| `INCOMPLETE` | Incompleto | both missing |
| `OUTLIER_VALUE` / `OUTLIER_QUANTITY` | Valor / Quantidade atípica (válida) | far in the product's high tail (≥ 4 half-IQRs above its median, in log scale) with the implied price within 100× of the median — usually a genuinely large value, **not** an error. The tail is the product's WHOLE history, so the tier follows the product's trend over time; and "within 100×" is wide (~10% of COMTRADE's outliers are ≥ 10× off the median price) |
| `PROBLEMATIC_VALUE` / `PROBLEMATIC_QUANTITY` | Valor / Quantidade problemática (provável erro) | implied price (value ÷ quantity) >100× or <1/100× the product median ⇒ likely typo, blamed on the more anomalous measure. Two-sided: 83% of `PROBLEMATIC_QUANTITY` rows are quantities BELOW the median (the weight = 1 placeholder) |
...
> ⚠️ **`UNSCORED` is the majority flag by ROWS and a small one by VALUE, and both facts matter.** Measured on prod 2026-09-24 (IBGE value weighted by IGP-DI, trade by nominal US$) — PEVS **81,7%** of rows / **0,74%** of value · COMEX 65,2% / 0,48% · COMTRADE 64,6% / 3,17% · PAM 66,3% / **8,98%** · PPM 87,2% / **10,38%**. The detector therefore examines over 99% of the money in PEVS and COMEX, ~96% in COMTRADE, and ~91% / ~90% in PAM / PPM. Most of the rows it skips are numerous and economically negligible: in the IBGE surveys mostly a município that measurably produced **zero** (SIDRA's `-`), and in the trade banks shipments under the US$ 100k floor. The PAM/PPM exception is not negligible and not noise: it is **1974–1979**, years the IPCA (the index the detector scores on) does not reach. Until v1.89.0 this note said "over 96% in every banco", because the value was weighted by IPCA and those rows counted as R$ 0 — see `docs/audits/qualidade_dados_audit_2026-09-24.md`. Reading the row share alone says two thirds of the acervo went unexamined — true of the *rows*, false of the *subject*.
>
> The absence tags (`MISSING_VALUE`, `MISSING_QUANTITY`, `INCOMPLETE`) never appear in the IBGE surveys: when SIDRA has no data (`...`), the whole cell is blank — quantity and value together — and Gold drops it before flagging. In PAM that is mostly a município that did not exist yet, or a produto the survey did not cover that year; in PEVS, a whole município-year with no extraction data at all.
