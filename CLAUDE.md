...
- **A number that is arithmetically right can answer the wrong question — four rules, all learned the hard way.** These are the reading layer's counterpart to the dbt `quality_value_floor`, and every one of them was found in production, not in review. They live in `frontend/src/ui/seriesUtils.js` (JS) and `src/embrapa_dashboard/webapi/measures.py` (Python), with sweeps in `frontend/src/ui/absenceGuard.test.js`, `tests/test_absence_guard.py`, `tests/test_cross_empty_codes_guard.py` and `tests/test_ratio_same_rows_guard.py`.
