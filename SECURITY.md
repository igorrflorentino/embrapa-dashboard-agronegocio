...
Use GitHub's [Security Advisories](https://github.com/igorrflorentino/embrapa-dashboard-agronegocio/security/advisories/new) feature to report privately.
-->
...
- **Infrastructure**: Cloud Run with mandatory IAM, 4 Service Accounts with minimal roles, budget alerts. No principal reads Secret Manager at project level: each secret grants only its own consumer. The default compute SA (the Cloud Build builder) holds `roles/run.builder`, not the GCP default `roles/editor` (since 2026-09-24).
- **Audit**: Admin Activity logs always; Data Access logs on for BigQuery and Secret Manager, so every read of a secret value is attributable (since 2026-09-24).
